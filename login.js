document.addEventListener("DOMContentLoaded", () => {
  ...
  form.addEventListener("submit", async (e) => {
    ...
    try {
      const response = await fetch("https://portal.gradients.academy/users/token/", {
        method: "POST",
      ...
      if (!response.ok || !data.access || !data.user) {
        const msg = data?.detail || "Ошибка входа. Проверьте данные.";
      ...
      switch (role) {
        ...
        case "participant":
          ...
          break;
        case "representive":
          window.location.href = "representive/index.html";
          break;
        default:
          messageContainer.textContent = "Неизвестная роль пользователя.";
          messageContainer.classList.add("text-red-500");
      }
    } catch (error) {
